...
function InquiryForm({ id, animateOnView = true, hoverLift = true }) {
  ...
  return (
    <>
      <motion.div
        className="rounded-[12px] border border-white/10 bg-[#253e35]/80 px-[26px] py-[30px]"
        id={id}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <form onSubmit={handleSubmit} aria-label="Institution inquiry form">
              <div className="mb-3">
                <label
                  className="mb-[5px] block text-[0.8rem] uppercase tracking-[0.5px] text-white/65"
                  htmlFor={`name-${id}`}
                >
                  Your Name
                </label>
                <input
                  className="w-full rounded-[8px] border border-white/10 bg-[#1a2f28]/90 px-[14px] py-[11px] text-sm text-white placeholder:text-white/15 focus:border-[#e2fea5] focus:outline-none focus:ring-3 focus:ring-[rgba(226,254,165,0.08)]"
                  type="text"
                  id={`name-${id}`}
                  name="name"
              ...
              <motion.button
                ...
                className="mt-2 inline-flex w-full items-center justify-center rounded-[8px] bg-[#e2fea5] px-8 py-4 text-base text-[#0d1f1a] transition hover:bg-[#c8e88a] hover:shadow-[0_4px_16px_rgba(226,254,165,0.25)] disabled:opacity-70 disabled:cursor-not-allowed"
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            role="alert"
            aria-live="assertive"
            className={`mt-3 rounded-[12px] border px-4 py-3 text-sm font-semibold shadow-lg ${
              error
