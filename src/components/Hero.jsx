import React from "react";
import { motion, useReducedMotion } from "framer-motion";
import InquiryForm from "./InquiryForm";
import { cardVariants, sectionVariants, staggerContainer } from "../lib/motion";
import { CheckCircle } from "lucide-react";

function Hero() {
  const shouldReduceMotion = useReducedMotion();

  const trustItems = [
    "Education-Only Focus",
    "Google & Meta Certified",
    "No Long-Term Lock-in",
    "Results in 30 Days",
  ];

  const formTransition = shouldReduceMotion
    ? { duration: 0.25, ease: "easeOut" }
    : { duration: 0.55, ease: [0.22, 1, 0.36, 1], delay: 0.15 };

  return (
    <section
      className="relative overflow-hidden pb-16 pt-[100px] md:pt-[132px] md:pb-[72px]"
      id="hero"
      aria-label="Hero section"
    >
      {/* Background image */}
      <div
        className="pointer-events-none absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: "url('/hero_bg2.png')" }}
        aria-hidden="true"
      />

      {/* Overlays */}
      <div
        className="pointer-events-none absolute inset-0 bg-[#0d1f1a]/80"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-b from-[#0d1f1a]/50 via-transparent to-[#0d1f1a]/90"
        aria-hidden="true"
      />

      {/* Radial glow */}
      <div
        className="pointer-events-none absolute left-1/2 top-[-100px] h-[500px] w-[700px] -translate-x-1/2 bg-[radial-gradient(ellipse,rgba(226,254,165,0.06)_0%,transparent_70%)]"
        aria-hidden="true"
      />

      <div className="relative z-[1] mx-auto max-w-[1200px] px-[18px] md:px-7">
        {/* ── Main grid: text | desktop-form ── */}
        <div className="grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_400px] lg:gap-[52px]">
          {/* ── Left: copy ── */}
          <motion.div
            className="max-w-[620px]"
            variants={staggerContainer}
            initial="hidden"
            animate="show"
          >
            <motion.span
              className="mb-[22px] inline-block rounded-full border border-[rgba(226,254,165,0.25)] bg-[rgba(226,254,165,0.12)] px-[14px] py-[6px] text-[0.78rem] font-semibold uppercase tracking-[0.4px] text-[#e2fea5]"
              variants={cardVariants}
            >
              Education Specialist Agency{" "}
              <span className="hidden md:inline">- Chennai</span>
            </motion.span>

            <motion.h1 variants={sectionVariants}>
              <span className="mb-[22px] block font-['Dela_Gothic_One'] text-[1.9rem] leading-[1.1] text-white sm:text-[2.1rem] md:text-[clamp(2.2rem,5vw,3.6rem)]">
                Stop Losing Admissions.
                <br />
                <span className="text-[#e2fea5]">Start Filling Seats.</span>
              </span>
            </motion.h1>

            <motion.p
              className="mb-[28px] max-w-[520px] text-[0.95rem] leading-7 text-white/78 md:mb-[34px] md:text-[1.05rem]"
              variants={cardVariants}
            >
              Chennai's specialist admissions marketing agency for schools,
              colleges, coaching institutes & preschools. We run high-converting
              admission campaigns that fill seats — every season.
            </motion.p>

            <motion.div
              className="mb-[30px] flex flex-col gap-3 sm:flex-row sm:flex-wrap md:mb-[38px]"
              variants={cardVariants}
            >
              <motion.a
                href="#services"
                className="inline-flex w-full items-center justify-center rounded-[8px] border-[1.5px] border-[rgba(226,254,165,0.25)] bg-transparent px-8 py-4 text-base font-semibold text-[#e2fea5] transition hover:border-[#e2fea5] hover:bg-[rgba(226,254,165,0.12)] sm:w-auto"
                aria-label="See how our admission marketing services work"
                whileHover={{ y: -2 }}
                whileTap={{ scale: 0.98 }}
              >
                See How It Works
              </motion.a>
            </motion.div>

            <motion.div
              className="grid grid-cols-1 gap-4 md:grid-cols-2"
              aria-label="Trust signals"
              variants={staggerContainer}
            >
              {trustItems.map((item) => (
                <motion.span
                  className="flex items-center gap-[7px] text-[0.82rem] font-medium text-white/70"
                  key={item}
                  variants={cardVariants}
                >
                  <span className="text-[#e2fea5]" aria-hidden="true">
                    <CheckCircle size={20} />
                  </span>
                  {item}
                </motion.span>
              ))}
            </motion.div>
          </motion.div>

          {/* ── Right: desktop form (lg+) ── */}
          <div id="contact-form" className="hidden lg:block">
            <motion.div
              className="transform-gpu will-change-transform"
              initial={{ opacity: 0, y: 40 }}
              animate={{ opacity: 1, y: 0 }}
              transition={formTransition}
            >
              <InquiryForm
                id="inquiry"
                animateOnView={false}
                hoverLift={false}
              />
            </motion.div>
          </div>
        </div>

        {/* ── Mobile / tablet form (below lg) ──
            Key fix: use `animate` driven by `initial` instead of `whileInView`
            so the animation fires reliably even when the element starts in-view.
        ── */}
        <motion.div
          className="mt-10 lg:hidden"
          id="contact-form-mobile"
          initial={{ opacity: 0, y: 28 }}
          animate={{ opacity: 1, y: 0 }}
          transition={
            shouldReduceMotion
              ? { duration: 0.25, ease: "easeOut" }
              : { duration: 0.6, ease: [0.22, 1, 0.36, 1], delay: 0.35 }
          }
        >
          <InquiryForm id="inquiry-mobile" />
        </motion.div>
      </div>
    </section>
  );
}

export default Hero;
